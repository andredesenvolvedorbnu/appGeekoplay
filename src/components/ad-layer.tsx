...
export function AdLayer(){
 ...
 const media=<div onClick={()=>{if(ad.cta_url)void click()}} className={`relative flex w-full items-center justify-center overflow-hidden bg-black/30 ${ad.cta_url?'cursor-pointer':''} ${isPopup?'min-h-40 max-h-[52dvh] p-2 sm:max-h-[58vh]':'aspect-[16/9] sm:aspect-[16/8]'}`}>
   {ad.image_url?<img src={ad.image_url} alt={ad.title} className={isPopup?'block h-auto max-h-[50dvh] w-auto max-w-full object-contain object-center sm:max-h-[56vh]':'block h-full w-full object-contain object-center'}/>:<div className="flex h-full min-h-40 w-full flex-col items-center justify-center gap-2 text-slate-500"><ImageIcon size={28}/><span className="text-xs">Imagem do anúncio</span></div>}
 ...
 const content=<div className={`mx-auto w-full max-w-full overflow-hidden rounded-2xl border border-orange-500/25 bg-[#171b22] shadow-2xl ${isPopup?'max-h-[92dvh] overflow-y-auto overscroll-contain':''}`}>{media}<div className="p-4"><p className="text-[10px] font-black uppercase tracking-[.18em] text-orange-300">Patrocinado</p><h3 className="mt-1 break-words text-lg font-black">{ad.title}</h3>{ad.description&&<p className="mt-2 break-words text-sm leading-6 text-slate-400">{ad.description}</p>}{ad.has_promo_code&&ad.promo_code&&<div className="mt-3 rounded-xl border border-geek-line bg-geek-soft p-3"><p className="text-[10px] uppercase text-slate-500">Código promocional</p><div className="mt-1 flex items-center justify-between gap-3"><b className="truncate text-orange-300">{ad.promo_code}</b><button onClick={()=>void copyCode()} className="flex shrink-0 items-center gap-1 text-xs font-bold"><Copy size={14}/>Copiar</button></div>{ad.promo_instructions&&<p className="mt-1 break-words text-xs text-slate-500">{ad.promo_instructions}</p>}</div>}{ad.cta_url&&<button onClick={()=>void click()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-geek-orange px-4 py-3 font-black">{ad.cta_label||'Saiba mais'}<ExternalLink size={16}/></button>}{ad.display_seconds>0&&<p className="mt-2 text-center text-[10px] text-slate-600">Este anúncio fecha automaticamente.</p>}</div></div>;
