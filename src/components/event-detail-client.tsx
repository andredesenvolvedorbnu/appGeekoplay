...
export function EventDetailClient({eventId}:{eventId:string}){
 ...
 useEffect(()=>{void load();const ch=supabase.channel(`event-detail-${eventId}`).on('postgres_changes',{event:'*',schema:'public',table:'event_attendees',filter:`event_id=eq.${eventId}`},()=>void load()).on('postgres_changes',{event:'UPDATE',schema:'public',table:'events',filter:`id=eq.${eventId}`},()=>void load()).subscribe();return()=>{void supabase.removeChannel(ch)}},[eventId,supabase]);
 ...
 return <div className="mx-auto max-w-5xl space-y-5 px-3 pb-10 sm:px-4">
  ...
  <section className="overflow-hidden rounded-3xl border border-geek-line bg-geek-panel">
   ...
   <div className="p-5 sm:p-7"><div className="flex flex-col gap-4 md:flex-row md:items-start"><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2">{event.category&&<span className="rounded-full bg-orange-500/10 px-2.5 py-1 text-xs text-orange-300">{event.category}</span>}{ended&&<span className="rounded-full bg-slate-500/10 px-2.5 py-1 text-xs text-slate-400">Encerrado</span>}</div><h1 className="mt-3 text-2xl font-black sm:text-3xl">{event.title}</h1><div className="mt-4 grid gap-2 text-sm text-slate-400"><div className="flex items-start gap-2"><CalendarDays size={17} className="mt-0.5 shrink-0"/><span>{new Date(event.starts_at).toLocaleString('pt-BR',{dateStyle:'long',timeStyle:'short'})}{event.ends_at?` até ${new Date(event.ends_at).toLocaleString('pt-BR',{dateStyle:'long',timeStyle:'short'})}`:''}</span></div><div className="flex items-start gap-2">{event.is_online?<><Video size={17} className="mt-0.5 shrink-0"/><span>Evento online</span></>:<><MapPin size={17} className="mt-0.5 shrink-0"/><span>{[event.venue_name,event.address,event.city,event.state].filter(Boolean).join(' · ')||'Local a confirmar'}</span></>}</div></div></div><div className="flex flex-wrap gap-2 md:max-w-[260px]">{!ended&&<button onClick={toggleGoing} disabled={busy} className={`flex-1 rounded-xl px-5 py-3 text-sm font-black disabled:opacity-60 ${attending?'border border-emerald-500/40 bg-emerald-500/10 text-emerald-300':'bg-geek-orange text-white'}`}>{busy?'Aguarde...':attending?<span className="inline-flex items-center gap-2"><Check size={17}/>Vou participar</span>:'Confirmar presença'}</button>}{ended&&attending&&!feedbackDone&&<Link href={`/eventos/${event.id}/avaliar`} className="rounded-xl bg-geek-orange px-5 py-3 text-sm font-black">Avaliar evento</Link>}{ended&&feedbackDone&&<span className="rounded-xl border border-geek-line px-4 py-3 text-sm text-emerald-300">Avaliação enviada</span>}{event.external_url&&<a href={event.external_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-geek-line px-4 py-3 text-sm font-bold"><ExternalLink size={16}/>Site oficial</a>}</div></div>{event.description&&<p className="mt-6 max-w-3xl whitespace-pre-wrap text-sm leading-7 text-slate-300 sm:text-base">{event.description}</p>}{message&&<p className="mt-4 rounded-xl border border-geek-line bg-geek-soft px-4 py-3 text-sm text-slate-300">{message}</p>}</div>
  </section>
