...
type Benefit={id:string;title:string;sort_order:number};
...
function monthsForPlan(plan:Plan){return Math.max(1,Math.round(plan.duration_days/30))}
...
export function PremiumClient(){
 ...
 async function load(){
  ...
  const [{data:s},{data:planRows},{data:benefitRows},{data:p},{data:r}]=await Promise.all([
   ...
   supabase.from('premium_benefits').select('id,title,sort_order').eq('is_active',true).order('sort_order',{ascending:true}),
  ...
  setSettings(s as Settings|null);setPlans(nextPlans);setBenefits((benefitRows||[]) as Benefit[]);setSelectedId(current=>current||nextPlans[0]?.id||null);setProfile((p as Profile|null)||{is_pro:false,pro_expires_at:null});setRequest(r as Request|null);setLoading(false)
 }
 ...
 const baselinePlan=plans[0]||null;
