...
import { Award, Bell, CalendarDays, Compass, Crown, Gamepad2, HelpCircle, Home, IdCard, LibraryBig, LogOut, Mail, Menu, MessageSquare, Newspaper, PlusCircle, Rocket, Search, Share2, ShieldCheck, Sparkles, Store, Trophy, UserRound, Users, X } from 'lucide-react';
...
const main = [
  ...
  ['Guia GeekoPlay', '/guia', HelpCircle]
] as const;
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-geek-bg text-slate-100">
      ...
      <aside className="fixed bottom-0 right-0 top-14 hidden w-72 overflow-y-auto border-l border-geek-line bg-geek-panel p-4 xl:block">
        ...
        <Link href="/recap" className="mt-3 block rounded-2xl border border-geek-line bg-geek-panel p-4 transition hover:border-orange-500/40"><div className="flex items-center gap-2"><Sparkles size={17} className="text-geek-orange"/><b>Meu Recap Geek</b></div><p className="mt-2 text-sm text-slate-400">Reviva sua metade do ano geek quando quiser.</p></Link><Link href="/guia" className="mt-3 block rounded-2xl border border-geek-line bg-geek-panel p-4 transition hover:border-orange-500/40"><div className="flex items-center gap-2"><HelpCircle size={17} className="text-geek-orange"/><b>Guia GeekoPlay</b></div><p className="mt-2 text-sm text-slate-400">Entenda XP, Pulses, reações, eventos, PRO e muito mais.</p></Link>
