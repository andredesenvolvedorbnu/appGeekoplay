...
import { CalendarHeart,ChevronLeft,ChevronRight,Copy,Flag,Gamepad2,Heart,HelpCircle,IdCard,Image as ImageIcon,Loader2,MessageCircle,MoreHorizontal,Pencil,Rocket,Send,Share2,Trash2,Video,X } from 'lucide-react';
...
type CardData={title?:string;subtitle?:string;rarity?:string;atk?:number;def?:number;description?:string;category?:string;photo_url?:string;event_plan_id?:string;event_date?:string;location?:string;note?:string;collection_item_id?:string;item_type?:string;status?:string;status_label?:string;notes?:string;image_url?:string};
...
type PostMedia={id:string;post_id:string;media_type:'image'|'video';url:string;storage_path:string|null;position:number};
...
type ModerationResult={decision:'allow'|'review'|'block';caseId?:string;categories?:string[];reasons?:string[];summary?:string};
type ModerationNotice={caseId:string;decision:'review'|'block';reasons:string[];summary:string};

async function canvasDataUrl(source:CanvasImageSource,width:number,height:number){
 const maxSide=720;const scale=Math.min(1,maxSide/Math.max(width,height));const canvas=document.createElement('canvas');
...
async function imageModerationPreview(file:File){
 ...
 try{
  ...
  return await canvasDataUrl(image,image.naturalWidth||image.width,image.naturalHeight||image.height);
 }finally{URL.revokeObjectURL(url)}
}

async function videoModerationFrames(file:File){
 ...
 try{
  ...
  for(const ratio of [0.05,0.25,0.5,0.75,0.95]){
   ...
   await new Promise<void>((resolve,reject)=>{
    ...
    video.onseeked=()=>{window.clearTimeout(timer);resolve()};video.onerror=()=>{window.clearTimeout(timer);reject(new Error('video seek'))};video.currentTime=target;
   });
   frames.push(await canvasDataUrl(video,video.videoWidth||640,video.videoHeight||360));
  }
  return frames;
 }catch{return []}finally{video.removeAttribute('src');video.load();URL.revokeObjectURL(url)}
}

async function moderationImages(file:File|null){
 if(!file)return[];
 if(IMAGE_TYPES.includes(file.type)){try{return[await imageModerationPreview(file)]}catch{return[]}}
 if(VIDEO_TYPES.includes(file.type))return videoModerationFrames(file);
 return[];
}
...
function CollectionPost({post}:{post:Post}){
 const card=post.card_data||{};
 const href=card.collection_item_id?`/colecao/${card.collection_item_id}`:'#';
 return <div className="px-3 pb-4 sm:px-4"><Link href={href} className="mx-auto block w-full max-w-[520px] overflow-hidden rounded-[26px] border border-orange-400/30 bg-gradient-to-b from-[#1b1e26] to-[#111318] shadow-xl transition hover:border-orange-400/55">
  {(card.image_url||post.image_url)&&<div className="flex aspect-[16/10] items-center justify-center overflow-hidden bg-black/30"><img src={card.image_url||post.image_url||''} alt={card.title||'Item da coleção'} className="h-full w-full object-contain object-center"/></div>}
  <div className="p-4 sm:p-5">
   <div className="flex flex-wrap gap-2">{card.status_label&&<span className="rounded-full bg-orange-500/10 px-2.5 py-1 text-[10px] font-black text-orange-300">{card.status_label}</span>}{card.category&&<span className="rounded-full bg-white/5 px-2.5 py-1 text-[10px] text-slate-400">{card.category}</span>}{card.item_type&&<span className="rounded-full bg-white/5 px-2.5 py-1 text-[10px] text-slate-400">{card.item_type}</span>}</div>
   <h3 className="mt-3 break-words text-lg font-black text-slate-100 sm:text-xl">{card.title||'Item da coleção'}</h3>
   {card.notes&&<p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-400">{card.notes}</p>}
   <div className="mt-4 flex items-center justify-between gap-3 border-t border-white/10 pt-3 text-xs"><span className="font-black text-orange-300">Minha Coleção</span><span className="text-slate-500">Abrir card</span></div>
  </div>
 </Link></div>
}
...
function PostGallery({items}:{items:PostMedia[]}){
 const ref=useRef<HTMLDivElement>(null);
 if(items.length===0)return null;
 const scroll=(dir:number)=>ref.current?.scrollBy({left:dir*ref.current.clientWidth,behavior:'smooth'});
 return <div className="relative overflow-hidden bg-black/30">
  <div ref={ref} className="flex snap-x snap-mandatory overflow-x-auto scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
   {items.map((media,index)=><div key={media.id} className="flex min-w-full snap-center items-center justify-center">
    <img src={media.url} alt={`Foto ${index+1} da publicação`} className="block h-auto max-h-[720px] w-auto max-w-full object-contain object-center"/>
   </div>)}
  </div>
  {items.length>1&&<><button type="button" onClick={()=>scroll(-1)} className="absolute left-2 top-1/2 hidden -translate-y-1/2 rounded-full bg-black/55 p-2 text-white sm:grid" aria-label="Foto anterior"><ChevronLeft size={20}/></button><button type="button" onClick={()=>scroll(1)} className="absolute right-2 top-1/2 hidden -translate-y-1/2 rounded-full bg-black/55 p-2 text-white sm:grid" aria-label="Próxima foto"><ChevronRight size={20}/></button><span className="absolute right-3 top-3 rounded-full bg-black/60 px-2 py-1 text-[10px] font-bold text-white">{items.length} fotos</span></>}
 </div>
}
...
export function FeedClient(){
 ...
 const [selectedFile,setSelectedFile]=useState<File|null>(null);const [previewUrl,setPreviewUrl]=useState<string|null>(null);const [selectedImages,setSelectedImages]=useState<File[]>([]);const [imagePreviews,setImagePreviews]=useState<string[]>([]);const [postMedia,setPostMedia]=useState<Record<string,PostMedia[]>>({});
 ...
 const [moderationNotice,setModerationNotice]=useState<ModerationNotice|null>(null);const [appealText,setAppealText]=useState('');const [appealing,setAppealing]=useState(false);const [appealSent,setAppealSent]=useState(false);
 ...
 async function reload(showLoader=true){
  ...
  if(safe.length){
   const {data:mediaRows}=await supabase.from('post_media').select('id,post_id,media_type,url,storage_path,position').in('post_id',safe.map(p=>p.id)).order('position',{ascending:true});
   const mediaMap:Record<string,PostMedia[]>={};((mediaRows||[]) as PostMedia[]).forEach(m=>(mediaMap[m.post_id]||=[]).push(m));setPostMedia(mediaMap);
  }else setPostMedia({});
 ...
 async function checkModeration(contentType:'post'|'comment',text:string,file:File|null,contentCategory?:string,parentId?:string){
  ...
  const mediaKind=file?(VIDEO_TYPES.includes(file.type)?'video':'image'):'none';
  const response=await fetch('/api/moderation/check',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contentType,text,category:contentCategory||null,mediaKind,images,parentId:parentId||null})});
  const data=await response.json();
  if(!response.ok)throw new Error(data?.error||'Falha na verificação de segurança.');
  return data as ModerationResult;
 }
 function showModeration(result:ModerationResult){
  if(!result.caseId)return;
  setAppealText('');setAppealSent(false);
  setModerationNotice({caseId:result.caseId,decision:result.decision==='review'?'review':'block',reasons:result.reasons||[],summary:result.summary||''});
 }
 async function submitAppeal(){
  if(!userId||!moderationNotice||appealText.trim().length<5)return;
  setAppealing(true);
  const {error}=await supabase.from('moderation_appeals').insert({case_id:moderationNotice.caseId,user_id:userId,message:appealText.trim()});
  if(error?.code==='23505'){setAppealSent(true);setMessage('Sua contestação já foi enviada para a equipe.')}
  else if(error)setMessage('Não foi possível enviar a contestação.')
  else{setAppealSent(true);setMessage('Contestação enviada. Os ADMs foram notificados.')}
  setAppealing(false);
 }
 ...
 function clearSelectedMedia(){
  ...
  setSelectedFile(null);setPreviewUrl(null);setSelectedImages([]);setImagePreviews([]);
 }
 function chooseFiles(files:File[]){
  setMessage('');clearSelectedMedia();
  ...
  if(videos.length){
   if(files.length>1){setMessage('Para vídeo, envie apenas 1 arquivo por publicação.');return}
   const file=videos[0];if(file.size>MAX_VIDEO){setMessage('O vídeo precisa ter no máximo 50 MB.');return}
   setSelectedFile(file);setPreviewUrl(URL.createObjectURL(file));return;
  }
  const valid=images.filter(file=>file.size<=MAX_IMAGE).slice(0,10);
  if(valid.length!==files.length&&files.some(file=>!IMAGE_TYPES.includes(file.type))){setMessage('Alguns arquivos foram ignorados. Use JPG, PNG, WEBP ou GIF.')}
  else if(images.some(file=>file.size>MAX_IMAGE))setMessage('Algumas imagens acima de 15 MB foram ignoradas.');
  if(images.length>10)setMessage('Limite de 10 fotos por publicação. As 10 primeiras foram selecionadas.');
 ...
 async function createPost(){
  ...
  try{
   setMessage('Verificando as Diretrizes da Comunidade...');
   let moderationFiles:File[]=[];
   if(selectedFile)moderationFiles=[selectedFile];
   else moderationFiles=selectedImages;
   const moderationImagesList=(await Promise.all(moderationFiles.map(file=>moderationImages(file)))).flat();
   const response=await fetch('/api/moderation/check',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contentType:'post',text:content.trim(),category:category||null,mediaKind:selectedFile?'video':selectedImages.length?'image':'none',images:moderationImagesList,parentId:null})});
   const moderation=await response.json() as ModerationResult;
   if(!response.ok)throw new Error((moderation as any)?.error||'Falha na verificação de segurança.');
   if(moderation.decision!=='allow'){showModeration(moderation);setMessage(moderation.decision==='review'?'Sua publicação ficou retida para revisão e não apareceu no feed.':'Sua publicação não foi publicada porque pode violar as Diretrizes da Comunidade.');return}
   setMessage('');
   let imageUrl:string|null=null;let videoUrl:string|null=null;const gallery:{url:string;path:string;position:number}[]=[];
   if(selectedFile){
    const uploaded=await uploadSingleMedia(selectedFile);uploadedPaths.push(uploaded.path);
    if(VIDEO_TYPES.includes(selectedFile.type))videoUrl=uploaded.url;else imageUrl=uploaded.url;
   }else if(selectedImages.length){
    for(let i=0;i<selectedImages.length;i++){
     const uploaded=await uploadSingleMedia(selectedImages[i]);uploadedPaths.push(uploaded.path);
     gallery.push({url:uploaded.url,path:uploaded.path,position:i});
    }
    imageUrl=gallery[0]?.url||null;
   }
   const {data:created,error}=await supabase.from('posts').insert({author_id:userId,content:content.trim()||null,image_url:imageUrl,video_url:videoUrl,category,post_type:'post'}).select('id').single();
   if(error||!created)throw error||new Error('post create');
   if(gallery.length>1){
    const {error:mediaError}=await supabase.from('post_media').insert(gallery.map(m=>({post_id:created.id,media_type:'image',url:m.url,storage_path:m.path,position:m.position})));
    if(mediaError){await supabase.from('posts').delete().eq('id',created.id);throw mediaError}
   }
   setContent('');clearSelectedMedia();if(fileInputRef.current)fileInputRef.current.value='';await reload();setMessage('Publicação criada com sucesso.');
  }catch(error){
   if(uploadedPaths.length)await supabase.storage.from('posts').remove(uploadedPaths);
   const detail=error&&typeof error==='object'&&'message' in error?String((error as {message?:string}).message):'';
   if(/verificar|segurança|moderation/i.test(detail))setMessage('Não foi possível concluir a verificação de segurança. A publicação não foi enviada; tente novamente.');
   else if(/row-level security|permission|jwt/i.test(detail))setMessage('Sua sessão não tem permissão para publicar. Saia da conta, entre novamente e tente de novo.');
   ...
   else setMessage('Não foi possível concluir a publicação. Nenhuma cópia incompleta foi mantida; tente novamente.');
 ...
 async function sendComment(postId:string){const text=(commentDrafts[postId]||'').trim();if(!userId||!text)return;setCommentBusy('send:'+postId);try{const moderation=await checkModeration('comment',text,null,undefined,postId);if(moderation.decision!=='allow'){showModeration(moderation);setMessage('Este comentário não foi publicado porque precisa de revisão de segurança.');return}const {error}=await supabase.from('comments').insert({post_id:postId,author_id:userId,content:text});if(error){setMessage('Não foi possível enviar o comentário.');return}setCommentDrafts(c=>({...c,[postId]:''}));await reload(false)}catch{setMessage('Não foi possível verificar este comentário agora. Ele não foi publicado.')}finally{setCommentBusy(null)}}
 ...
 return <div className="mx-auto max-w-2xl space-y-4 px-3 sm:px-4">
  <section className="rounded-2xl border border-geek-line bg-geek-panel p-4"><textarea value={content} onChange={e=>setContent(e.target.value)} maxLength={5000} placeholder="No que você está pensando, geek? 🎮" className="min-h-24 w-full resize-none rounded-xl border border-geek-line bg-geek-soft p-3 outline-none focus:border-geek-orange"/>{previewUrl&&<div className="relative mt-3 flex max-h-[560px] min-h-32 items-center justify-center overflow-hidden rounded-xl border border-geek-line bg-black/30"><button onClick={clearSelectedMedia} className="absolute right-2 top-2 z-10 rounded-full bg-black/70 p-1.5 text-white"><X size={16}/></button><video src={previewUrl} controls playsInline preload="metadata" className="block max-h-[560px] max-w-full object-contain"/></div>}{imagePreviews.length>0&&<div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{imagePreviews.map((url,index)=><div key={url} className="relative aspect-square overflow-hidden rounded-xl border border-geek-line bg-black/20"><img src={url} alt={`Prévia ${index+1}`} className="h-full w-full object-cover object-center"/><button type="button" onClick={()=>{URL.revokeObjectURL(url);setSelectedImages(v=>v.filter((_,i)=>i!==index));setImagePreviews(v=>v.filter((_,i)=>i!==index))}} className="absolute right-1.5 top-1.5 rounded-full bg-black/70 p-1 text-white" aria-label="Remover foto"><X size={14}/></button><span className="absolute bottom-1.5 left-1.5 rounded-full bg-black/65 px-2 py-0.5 text-[10px] font-bold text-white">{index+1}</span></div>)}</div>}<input ref={fileInputRef} type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" hidden onChange={e=>chooseFiles(Array.from(e.target.files||[]))}/><div className="mt-3 flex flex-wrap items-center gap-2"><button onClick={()=>fileInputRef.current?.click()} className="flex items-center gap-2 rounded-xl border border-geek-line bg-geek-soft px-3 py-2 text-xs">{selectedFile?<Video size={15}/>:<ImageIcon size={15}/>}Foto/Vídeo</button><Link href="/meu-card" className="flex items-center gap-2 rounded-xl border border-geek-line bg-geek-soft px-3 py-2 text-xs"><IdCard size={15}/>Meu Card</Link><button onClick={()=>setEventDialog(true)} className="flex items-center gap-2 rounded-xl border border-geek-line bg-geek-soft px-3 py-2 text-xs"><CalendarHeart size={15}/>Evento que vou</button><label className="ml-auto flex min-w-[150px] items-center gap-2 rounded-xl border border-geek-line bg-geek-soft px-3 py-2 text-xs text-slate-400"><span className="shrink-0 font-semibold text-slate-300">Categoria:</span><select aria-label="Categoria da publicação" value={category} onChange={e=>setCategory(e.target.value)} className="min-w-0 flex-1 rounded-lg bg-geek-panel px-2 py-1 text-xs text-slate-100 outline-none">{categories.slice(1).map(c=><option key={c} className="bg-geek-panel text-slate-100">{c}</option>)}</select></label><button onClick={createPost} disabled={sending||uploading||(!content.trim()&&!selectedFile&&!selectedImages.length)} className="rounded-xl bg-geek-orange px-4 py-2 text-sm font-bold disabled:opacity-50">{uploading?'Enviando mídia...':sending?'Publicando...':'Publicar'}</button></div>{selectedFile&&<p className="mt-2 text-[11px] text-slate-500">Vídeo · {(selectedFile.size/1024/1024).toFixed(1)} MB</p>}{selectedImages.length>0&&<p className="mt-2 text-[11px] text-slate-500">{selectedImages.length} {selectedImages.length===1?'foto selecionada':'fotos selecionadas'} · limite de 10 · compactação automática antes do envio</p>}{message&&<p className="mt-2 text-xs text-slate-300">{message}</p>}</section>
