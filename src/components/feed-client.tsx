...
import { Heart, MessageCircle, Image as ImageIcon, Gamepad2, CalendarDays, Layers3, Loader2, Send, X } from 'lucide-react';
...
export function FeedClient() {
  ...
  function chooseFile(file:File|null){
    ...
    if(!file.type.startsWith('image/')){setMessage('Por enquanto, envie uma imagem JPG, PNG ou WEBP.');return;}
  ...
  return <div className="mx-auto max-w-2xl px-3 sm:px-4 space-y-4">
    <section className="rounded-2xl border border-geek-line bg-geek-panel p-4">
      ...
      <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" onChange={e=>chooseFile(e.target.files?.[0]||null)}/>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <button type="button" className="rounded-xl border border-geek-line bg-geek-soft px-3 py-2 text-xs flex items-center gap-2"><Layers3 size={15}/>Meu Card</button>
        <button type="button" className="rounded-xl border border-geek-line bg-geek-soft px-3 py-2 text-xs flex items-center gap-2"><CalendarDays size={15}/>Evento que vou</button>
    ...
    {loading ? <div className="py-16 grid place-items-center text-slate-400"><Loader2 className="animate-spin"/></div> : visible.length===0 ? <div className="rounded-2xl border border-dashed border-geek-line bg-geek-panel p-10 text-center text-slate-400"><Gamepad2 className="mx-auto mb-3 text-geek-orange"/>Ainda não há publicações nesta categoria.</div> : visible.map(post=>{
      ...
      return <article key={post.id} className="rounded-2xl border border-geek-line bg-geek-panel overflow-hidden">
        ...
        {openComments.has(post.id)&&<div className="border-t border-geek-line bg-black/10 p-4 space-y-3">
          ...
          <div className="flex gap-2 pt-1"><input value={commentDrafts[post.id]||''} onChange={e=>setCommentDrafts(current=>({...current,[post.id]:e.target.value}))} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();void sendComment(post.id)}}} placeholder="Escreva um comentário..." className="min-w-0 flex-1 rounded-xl border border-geek-line bg-geek-soft px-3 py-2 text-sm outline-none focus:border-geek-orange"/><button onClick={()=>sendComment(post.id)} disabled={!(commentDrafts[post.id]||'').trim()} className="rounded-xl bg-geek-orange px-3 disabled:opacity-50" aria-label="Enviar comentário"><Send size={17}/></button></div>
        </div>}
      </article>
    })}
  </div>;
