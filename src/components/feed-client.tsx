...
import { CalendarHeart,Copy,Flag,Gamepad2,Heart,IdCard,Image as ImageIcon,Loader2,MessageCircle,MoreHorizontal,Pencil,Rocket,Send,Share2,Trash2,Video,X } from 'lucide-react';
...
export function FeedClient(){
 ...
 useEffect(()=>{void reload();const channel=supabase.channel('feed-live-v4').on('postgres_changes',{event:'*',schema:'public',table:'posts'},()=>void reload(false)).on('postgres_changes',{event:'*',schema:'public',table:'comments'},()=>void reload(false)).on('postgres_changes',{event:'*',schema:'public',table:'likes'},()=>void reload(false)).on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles'},()=>void reload(false)).subscribe();return()=>{void supabase.removeChannel(channel)}},[supabase]);
