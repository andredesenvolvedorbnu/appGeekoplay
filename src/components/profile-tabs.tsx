...
import { CalendarDays, Heart, Image as ImageIcon, LibraryBig, Loader2, Users } from 'lucide-react';
import { createClient } from '@/lib/supabase/client';

type Tab='publicacoes'|'curtidas'|'eventos'|'comunidades'|'colecao'|'sobre';
...
const tabs:[string,Tab][]=[['Publicações','publicacoes'],['Curtidas','curtidas'],['Eventos','eventos'],['Comunidades','comunidades'],['Coleção','colecao'],['Sobre','sobre']];
...
export function ProfileTabs(){
  ...
  useEffect(()=>{(async()=>{
    ...
    const [{data:ownPosts},{data:likes},{data:attendance},{data:memberships},{data:collection},{data:p}]=await Promise.all([
    ...
    setPosts((ownPosts||[]) as Post[]);setItems((collection||[]) as Item[]);setProfile((p||null) as Profile|null);
  ...
  return <section className="mx-auto mt-4 w-full max-w-4xl px-3 pb-8 sm:px-5">
    ...
    <div className="pt-4">{loading?<div className="grid place-items-center py-16"><Loader2 className="animate-spin text-geek-orange"/></div>:<>
      ...
      {tab==='eventos'&&(events.length===0?<Empty text="Você ainda não confirmou presença em eventos."/>:<div className="grid gap-3 sm:grid-cols-2">{events.map(e=><Link href="/eventos" key={e.id} className="overflow-hidden rounded-2xl border border-geek-line bg-geek-panel">{e.cover_url?<div className="aspect-[16/7] overflow-hidden bg-black/20"><img src={e.cover_url} alt="" className="h-full w-full object-cover object-center"/></div>:<div className="grid aspect-[16/7] place-items-center bg-geek-soft"><CalendarDays className="text-geek-orange"/></div>}<div className="p-4"><b>{e.title}</b><p className="mt-1 text-xs text-slate-500">{new Date(e.starts_at).toLocaleDateString('pt-BR')} · {[e.city,e.state].filter(Boolean).join(' / ')||'Local a definir'}</p></div></Link>)}</div>)}
      {tab==='comunidades'&&(communities.length===0?<Empty text="Você ainda não participa de comunidades."/>:<div className="grid gap-3 sm:grid-cols-2">{communities.map(c=><Link href="/comunidades" key={c.id} className="flex gap-3 rounded-2xl border border-geek-line bg-geek-panel p-4">{c.cover_url?<img src={c.cover_url} alt="" className="h-14 w-14 shrink-0 rounded-xl object-cover object-center"/>:<div className="grid h-14 w-14 shrink-0 place-items-center rounded-xl bg-geek-soft"><Users className="text-geek-orange"/></div>}<div className="min-w-0"><b className="block truncate">{c.name}</b><p className="text-xs text-orange-300">{c.category||'Comunidade'}</p>{c.description&&<p className="mt-1 line-clamp-2 text-xs text-slate-500">{c.description}</p>}</div></Link>)}</div>)}
      {tab==='colecao'&&(items.length===0?<Empty text="Sua coleção ainda está vazia."/>:<div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">{items.map(item=><Link href="/colecao" key={item.id} className="overflow-hidden rounded-2xl border border-geek-line bg-geek-panel">{item.image_url?<div className="aspect-[4/5] overflow-hidden bg-black/20"><img src={item.image_url} alt="" className="h-full w-full object-contain object-center"/></div>:<div className="grid aspect-[4/5] place-items-center bg-geek-soft"><LibraryBig className="text-geek-orange"/></div>}<div className="p-3"><b className="block truncate text-sm">{item.title}</b><p className="text-[10px] text-slate-500">{item.status} · {item.category||'Geek'}</p></div></Link>)}</div>)}
