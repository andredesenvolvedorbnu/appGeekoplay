  ...
  return <div className="fixed inset-x-0 top-0 z-[120] flex h-screen h-[100dvh] items-center justify-center overflow-hidden bg-black/80 px-2 py-2 sm:px-6 sm:py-6" role="dialog" aria-modal="true" aria-label={title}>
    <div className="flex max-h-full w-full max-w-3xl flex-col overflow-hidden rounded-3xl border border-geek-line bg-geek-panel shadow-2xl sm:w-[80vw] sm:max-w-4xl">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-4 sm:px-5">
        <p className="text-xs leading-5 text-slate-400 sm:text-sm">Arraste para escolher a área, use o zoom e confirme. A imagem abre em 80% para você poder diminuir ou aumentar o enquadramento sem deformar o arquivo.</p>
        <div ref={frameRef} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} className={`relative mx-auto mt-3 max-w-full cursor-grab touch-none overflow-hidden rounded-2xl border border-orange-500/30 bg-black/50 active:cursor-grabbing sm:mt-4 ${isPortrait?'':'w-full max-w-2xl'}`} style={{aspectRatio:String(aspect),width:isPortrait?`min(100%, ${aspect*46}dvh, ${aspect*32}rem)`:undefined}}>
      ...
      <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-geek-line bg-geek-panel px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 sm:flex-row sm:justify-between sm:px-5 sm:pb-5"><button type="button" onClick={reset} className="inline-flex items-center justify-center gap-2 rounded-xl border border-geek-line px-4 py-2.5 text-sm font-semibold"><RotateCcw size={16}/>Redefinir</button><div className="grid grid-cols-2 gap-2"><button type="button" onClick={onCancel} className="rounded-xl border border-geek-line px-4 py-2.5 text-sm font-semibold">Cancelar</button><button type="button" onClick={()=>void confirm()} disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-geek-orange px-4 py-2.5 text-sm font-semibold disabled:opacity-60">{saving?<Crop className="animate-pulse" size={16}/>:<Check size={16}/>}Confirmar recorte</button></div></div>
    </div>
  </div>;
}
