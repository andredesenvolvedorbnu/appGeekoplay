...
export function ImageCropper({file,aspect,title='Ajustar imagem',outputWidth=1600,onCancel,onConfirm}:Props){
  ...
  return <div className="fixed inset-0 z-[120] grid place-items-center overflow-y-auto bg-black/80 p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={title}>
    <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-geek-line bg-geek-panel p-4 shadow-2xl sm:w-[80vw] sm:max-w-4xl sm:p-5">
      ...
      <p className="mt-2 text-sm text-slate-400">Arraste para escolher a área, use o zoom e confirme. A imagem abre em 80% para você poder diminuir ou aumentar o enquadramento sem deformar o arquivo.</p>
      ...
      <div className="mt-4 flex items-center gap-3"><Minus size={17}/><input className="w-full accent-orange-500" type="range" min="0.5" max="3" step="0.01" value={zoom} onChange={e=>setZoom(Number(e.target.value))} aria-label="Zoom"/><Plus size={17}/><span className="w-14 text-right text-xs text-slate-400">{Math.round(zoom*100)}%</span></div>
