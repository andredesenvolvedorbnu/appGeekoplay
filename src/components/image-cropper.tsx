...
export function ImageCropper({file,aspect,title='Ajustar imagem',outputWidth=1600,onCancel,onConfirm}:Props){
  ...
  async function confirm(){
    const img=imageRef.current;const frame=frameRef.current;if(!img||!frame)return;
    setSaving(true);
    try{
      const fw=frame.clientWidth;const fh=frame.clientHeight;
      ...
      const sx=Math.max(0,(0-left)/scale);const sy=Math.max(0,(0-top)/scale);
  ...
  return <div className="fixed inset-0 z-[120] grid place-items-center bg-black/80 p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={title}>
    <div className="w-full max-w-3xl rounded-3xl border border-geek-line bg-geek-panel p-4 shadow-2xl sm:p-5">
      ...
      <div ref={frameRef} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} className="relative mx-auto mt-4 w-full max-w-2xl cursor-grab touch-none overflow-hidden rounded-2xl border border-orange-500/30 bg-black/50 active:cursor-grabbing" style={{aspectRatio:String(aspect)}}>
        <img ref={imageRef} src={url} alt="Imagem para recorte" draggable={false} className="pointer-events-none absolute left-1/2 top-1/2 max-w-none select-none" style={{transform:`translate(calc(-50% + ${pos.x}px), calc(-50% + ${pos.y}px)) scale(${zoom})`,width:'100%',height:'100%',objectFit:'cover'}}/>
