...
export function PhotoSourcePicker({onSelect,onSelectMany,multiple=false,accept='image/jpeg,image/png,image/webp',label='Carregar ou tirar uma foto',className='',cameraFacing='environment'}:Props){
  ...
  return <>
    ...
    <input ref={galleryRef} hidden type="file" accept={accept} multiple={multiple} onChange={e=>{const files=Array.from(e.target.files||[]);setOpen(false);if(multiple&&onSelectMany)onSelectMany(files);else onSelect(files[0]||null);if(galleryRef.current)galleryRef.current.value=''}}/>
    <input ref={cameraRef} hidden type="file" accept="image/*" capture={cameraFacing} onChange={e=>pick(e.target.files?.[0]||null)}/>
