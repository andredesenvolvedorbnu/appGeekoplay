...
export default async function AdminUsuariosPage() {
  ...
  const { data: users } = await supabase
    .from('profiles')
    .select('id,display_name,email,role,level,xp,is_pro,created_at')
    .order('created_at', { ascending: false })
    .limit(100);

  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <div><p className="text-sm font-bold text-geek-orange">ADMINISTRAÇÃO</p><h1 className="text-2xl sm:text-3xl font-black">Usuários</h1><p className="mt-2 text-sm text-slate-400">Contas cadastradas no GeekoPlay.</p></div>
